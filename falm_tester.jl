...
function get_sharpe(data, riskFreeRate=0.04, freq="1d")
    ...
    sharpe = (get_annual_returns(data, freq) - riskFreeRate) / get_volatility(data, freq)
...
algorithms_list = [init_FALM1, init_RiskyFALM, init_GreedyFALM, init_SafeFALM, init_NeutralFALM]
...
algo_names = ["FALM1", "RiskyFALM", "GreedyFALM", "SafeFALM", "NeutralFALM"]
...
markowitz_execute_orders!(context, data) = execute_orders!(context, data; propagateBalanceToPortfolio=false, executeOrder=singleExecutionFun)
function run_algo_on_dataset(dataset, algorithm, evalEvents; markowitz_growth=0.001)
    if algorithm == init_markowitz
        ...
        algo_context = @suppress AirBorne.Engines.DEDS.run(
            ...
            markowitz_execute_orders!,
            markowitz_expose_data;
            audit=true,
            verbose=false,
            initialEvents=evalEvents
        )
        result = @suppress summarizePerformance(dataset, algo_context, includeAccounts=false, riskFreeRate=0.04)
    else
...
for (letter, set) in datasets
    ...
    for (name, info) in set
        ...
        row = [6.0 get_annual_returns(sp500_prices, freq) get_sharpe(sp500_prices, 0.04, freq) get_max_drawdown(sp500_prices) get_volatility(sp500_prices, freq) get_VaR(sp500_prices)]
        table = vcat(table, row')
        ...
        row = [7.0 get_annual_returns(homogenous_prices, freq) get_sharpe(homogenous_prices, 0.04, freq) get_max_drawdown(homogenous_prices) get_volatility(homogenous_prices, freq) get_VaR(homogenous_prices)]
        table = vcat(table, row')
